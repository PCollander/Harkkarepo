...
# fuction that saves the search criteria to an ad hoc model
def save_preferences(form):
    haku = SuosittuHaku(Vuosi=form['vuosi'], Kk=form['kuukausi'],
                         Kunta=form['kunta'])
    haku.save()
    return

# initializes dicts for the time in question and calls a function that uses
# the dicts to count popularity of criteria, calls the template
def most_searched(request):
    haetut_vuodet = {}
    haetut_kuukaudet = {}
    haetut_kunnat = {}
    hakuvieraat = count_searchpage_visits()
    vuodet, kuukaudet, kunnat, haut = get_most_searched(haetut_vuodet,
                                                  haetut_kuukaudet,
                                                  haetut_kunnat)
    context = {
        'vuodet': vuodet,
        ...
        'kunnat': kunnat,
        'haut': haut,
        'hakuvieraat' : hakuvieraat,
    }
    return render(request, 'popular.html', context)

# function takes three empty dicts, fills them with the table of SuosittuHaku
# model, sorts the dicts and returns response to empty dict, a dict with less
# than five values or the biggest five values
def get_most_searched(haetut_vuodet, haetut_kuukaudet, haetut_kunnat):

    vuodet = SuosittuHaku.objects.values_list('Vuosi')
    kuukaudet = SuosittuHaku.objects.values_list('Kk')
    kunnat = SuosittuHaku.objects.values_list('Kunta')
    haut = len(vuodet)

    for vuosi in vuodet:
        if vuosi[0] in haetut_vuodet:
            haetut_vuodet[vuosi[0]] += 1
        else:
            haetut_vuodet[vuosi[0]] = 1

    for kuukausi in kuukaudet:
        kuukausi_lukuna = int(kuukausi[0])
        if KUUKAUDET[kuukausi_lukuna] in haetut_kuukaudet:
            haetut_kuukaudet[KUUKAUDET[kuukausi_lukuna]] += 1
        else:
            haetut_kuukaudet[KUUKAUDET[kuukausi_lukuna]] = 1

    for kunta in kunnat:
        if kunta[0] in haetut_kunnat:
            haetut_kunnat[kunta[0]] += 1
        else:
            haetut_kunnat[kunta[0]] = 1

    vuodet = Counter(haetut_vuodet)
    kuukaudet = Counter(haetut_kuukaudet)
    kunnat = Counter(haetut_kunnat)

    if len(haetut_vuodet) == 0:
        palautettavat_vuodet = "Ei haettuja vuosia"
    elif len(haetut_vuodet) >= 5:
        palautettavat_vuodet = vuodet.most_common(5)
    else:
        palautettavat_vuodet = vuodet.most_common(len(haetut_vuodet))

    if len(haetut_kuukaudet) == 0:
        palautettavat_kuukaudet = "Ei haettuja kuukausia"
    elif len(haetut_kuukaudet) >= 5:
        palautettavat_kuukaudet = kuukaudet.most_common(5)
    else:
        palautettavat_kuukaudet = kuukaudet.most_common(len(haetut_kuukaudet))

    if len(haetut_kunnat) == 0:
        palautettavat_kunnat = "Ei haettuja kuntia"
    elif len(haetut_kunnat) >= 5:
        palautettavat_kunnat = kunnat.most_common(5)
    else:
        palautettavat_kunnat = kunnat.most_common(len(haetut_kunnat))

    return palautettavat_vuodet, palautettavat_kuukaudet, \
           palautettavat_kunnat, haut

# saves the index of every visitor on the search page for the purpose of
# analysis, keeping count of the amount by giving them the correct number
def searchpage_visits():
    indeksi = len(Vierailu.objects.values_list('Vierailija'))
    if indeksi == 0:
        kerta = Vierailu(Vierailija = 1)
        kerta.save()
    else:
        kerta = Vierailu(Vierailija = indeksi + 1)
        kerta.save()
    return()

# returns the amount of visitors in the records for output purposes
def count_searchpage_visits():
    return len(Vierailu.objects.values_list('Vierailija'))